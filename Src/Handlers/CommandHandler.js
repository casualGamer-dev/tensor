...
const Command = async(client, message, args, cmd) => {
    ...
    try {
        ...
        if (command.config.ownerOnly == true && !conf.Discord.bot_owners.includes(message.author.id)) {
            return utils.error(message, `:warning: **This command is restricted to Developers of ${client.user.username}.** :warning:`, 10000);
        }

        if (command.config.restricted == true && !conf.Discord.bot_admins.includes(message.author.id)) {
