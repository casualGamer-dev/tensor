...
module.exports.run = async (client, message,args) => {
    ...
    if (!["greeting", "goodbye", "insult", "compliment"].includes(args[0])) {
        const ErrorEmbed = new Discord.MessageEmbed()
            .setTitle("**ERROR**")
    ...
    if (Meta.type == "pattern") {
        TrainEmbed = new Discord.MessageEmbed()
            .setTitle("**TRAIN**")
            .setDescription("Type endsess to stop training. As of now, type messages that pertain to " + args[0]);
        Session.add(message.author.id, args[0]);
    } else {
        var Dataset = JSON.parse(Fs.readFileSync("./Convo/Dataset.json"))[args[0]];
        var Response = Dataset.patterns[Math.floor(Math.random() * Dataset.patterns.length)];
        TrainEmbed = new Discord.MessageEmbed()
            .setTitle("**TRAIN**")
            .setDescription(`**Respond to:**\n${Response}`)
        Session.addResponse(message.author.id, args[0], Dataset, Response);
    }
